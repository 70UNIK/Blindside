SMODS.Consumable {
    ...
    pos = {x=0, y=6},
    config = {extra = {round = 2, charge = 2, xmult = 3, active = false}},
    ...
    use = function(self, card, area, copier)
        ...
        card_eval_status_text(card, 'extra', nil, nil, nil, {message = localize('k_plus_trinity'), colour = G.C.RED, card = card})
    end,
    load = function(self,card,card_table,other_card)
        local eval = function(card) return card.ability.extra.active or card.ability.extra.charge >= card.ability.extra.round end
        juice_card_until(card, eval, true)
    end,
